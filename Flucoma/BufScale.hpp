...
#include <flucoma/clients/nrt/BufScaleClient.hpp>
...
namespace Flucoma
{
// bufscale's parameter enum is anonymous; alias its namespace instead of a
// `using Idx = ...ParamIndex` enum alias.
namespace bufscale = fluid::client::bufscale;

/* Offline linear rescaling of the sample values of a sound file read from the
 * working folder: maps [input low, input high] to [output low, output high],
 * with optional clipping. Runs in a worker thread when playback starts and
 * whenever "Process" is toggled on.
 */
struct BufScale
{
  halp_meta(name, "BufScale")
  halp_meta(c_name, "fluid_bufscale")
  halp_meta(category, "Audio/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description,
            "Rescale the sample values of a sound file from an input range "
            "to an output range, with optional clipping")
  halp_meta(uuid, "A56F350E-2902-4E8E-B29F-423B1CCB097B")

  using Client
      = fluid::client::ClientWrapper<fluid::client::bufscale::BufScaleClient>;
  ...
  struct ins
  {
    ...
    // The range parameters are unconstrained in flucoma (feature data can be
    // any magnitude); give them explicit bipolar UI ranges.
    FluCoMa::fluid_float_param_ranged<Client, bufscale::kInLow, -1000., 1000., 0.>
        inputLow;
    FluCoMa::fluid_float_param_ranged<Client, bufscale::kInHigh, -1000., 1000., 1.>
        inputHigh;
    FluCoMa::fluid_float_param_ranged<Client, bufscale::kOutLow, -1000., 1000., 0.>
        outputLow;
    FluCoMa::fluid_float_param_ranged<Client, bufscale::kOutHigh, -1000., 1000., 1.>
        outputHigh;
    FluCoMa::fluid_param_for<Client, bufscale::kClip> clipping;

    FluCoMa::fluid_long_param_ranged<Client, bufscale::kStartFrame, 0, 100000000, 0>
        startFrame;
    FluCoMa::fluid_auto_toggle<Client, bufscale::kNumFrames> allFrames;
    FluCoMa::fluid_long_param_ranged<Client, bufscale::kNumFrames, 1, 100000000, 1>
        numFrames;
    FluCoMa::fluid_long_param_ranged<Client, bufscale::kStartChan, 0, 64, 0>
        startChan;
    FluCoMa::fluid_auto_toggle<Client, bufscale::kNumChans> allChans;
    FluCoMa::fluid_long_param_ranged<Client, bufscale::kNumChans, 1, 64, 1>
        numChans;
  } inputs;
  ...
  struct worker_t
  {
    ...
    static std::function<void(BufScale&)>
    work(std::shared_ptr<ins> in_ptr, double rate)
    {
      ...
      auto fail = [](std::string msg) {
        return [msg = std::move(msg)](BufScale& self) {
      ...
      host.set_input_buffer<bufscale::kSource>(std::move(source));

      if(in.destination.value.empty())
        return fail("No destination file name");

      auto dest = host.make_output_buffer<bufscale::kDest>();
      ...
      return [f = in.folder.value, s = in.source.value](BufScale& self) {
