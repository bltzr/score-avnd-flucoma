...
/* Offline transient-based slicer (de-clicking model) over a sound file from
 * the working folder. Slice positions go to the "Slices (s)" value output
 * (seconds) and to <folder>/<indices file> as sample positions in a float32
 * WAV — the FluCoMa interchange format.
 */
struct BufTransientSlice
{
  halp_meta(name, "BufTransientSlice")
  halp_meta(c_name, "fluid_buftransientslice")
  halp_meta(category, "Analysis/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description,
            "Transient-based slicer (de-clicking model): segments a sound "
            "file and outputs the slice positions")
  halp_meta(uuid, "5D2D235B-D8BB-460D-8BA5-866CA0569A32")

  // The non-threaded NRT adaptor is itself a complete client; our worker
  // provides the threading.
  using Client = fluid::client::NRTTransientSliceClient;
  ...
  struct ins
  {
    ...
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("order")> order;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("blockSize")>
        blockSize;
    // "padSize" is the model's analysis padding, unrelated to the NRT
    // wrapper's FFT "padding" enum (which slicers don't have).
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("padSize")> padding;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("skew")> skew;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("threshFwd")>
        threshFwd;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("threshBack")>
        threshBack;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("windowSize")>
        windowSize;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("clumpLength")>
        clumpLength;
  ...
  struct worker_t
  {
    ...
    static std::function<void(BufTransientSlice&)>
    work(std::shared_ptr<ins> in_ptr, double rate)
    {
      ...
      auto fail = [](std::string msg) {
        return [msg = std::move(msg)](BufTransientSlice& self) {
      ...
      return [out = std::move(out), f = in.folder.value,
              s = in.source.value](BufTransientSlice& self) {
