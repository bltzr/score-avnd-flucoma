...
/* Applies a fitted robust scaling to incoming points, every tick.
 * The model hot-reloads when its file changes on disk — refit with
 * RobustScaleFit while playing and the scaling follows.
 *
 * Feed the input with a feature vector and get the scaled vector out;
 * the Inverse toggle maps scaled values back to the original range.
 */
...
struct RobustScalePoint
{
  halp_meta(name, "RobustScalePoint")
  halp_meta(c_name, "fluid_robustscalepoint")
  halp_meta(category, "Analysis/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description, "Scales incoming points with a fitted robust scaling")
  halp_meta(uuid, "996D282A-BC7B-4D19-A97F-3C7D8D586EE5")

  struct ins
  {
    halp::folder_port<"Folder"> folder;
    halp::folder_combobox<"Model file", "Folder", "json", "robustscale.json"> model_file;
  ...
  struct worker_t
  {
    ...
    static std::function<void(RobustScalePoint&)>
    work(std::shared_ptr<load_request> rq)
    {
      ...
      return [m = std::move(m), err = std::move(err)](RobustScalePoint& self) {
