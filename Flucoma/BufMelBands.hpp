...
/* Offline mel band analysis of a sound file read from the working folder.
 * Runs in a worker thread when playback starts and whenever "Process" is
 * toggled on. The feature buffer (one channel per band, one frame per
 * analysis hop) is written to <folder>/<features file> as a float32 WAV whose
 * sample rate encodes the analysis hop rate.
 */
struct BufMelBands
{
  halp_meta(name, "BufMelBands")
  halp_meta(c_name, "fluid_bufmelbands")
  halp_meta(category, "Analysis/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description, "Mel band magnitudes of a sound file")
  halp_meta(uuid, "7326A904-0E57-4CDA-A0ED-984E9AA38873")
  ...
  using Client = fluid::client::NRTMelBandsClient;
  ...
  struct ins
  {
    ...
    halp::lineedit<"Features file", "melbands.wav"> featuresFile;
    ...
    FluCoMa::fluid_long_max_param<
        Client, FluCoMa::pidx<Client>("numBands"), 40, max_bands>
        numBands;
    ...
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("normalize")>
        normalize;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("scale")> scale;
  ...
  struct worker_t
  {
    ...
    static std::function<void(BufMelBands&)>
    work(std::shared_ptr<ins> in_ptr, double rate)
    {
      ...
      auto fail = [](std::string msg) {
        return [msg = std::move(msg)](BufMelBands& self) {
      ...
      return
          [frames, f = in.folder.value, s = in.source.value](BufMelBands& self) {
