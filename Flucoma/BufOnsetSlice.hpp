...
/* Offline spectral-difference onset slicer over a sound file from the working
 * folder. Slice positions go to the "Slices (s)" value output (seconds) and to
 * <folder>/<indices file> as sample positions in a float32 WAV — the FluCoMa
 * interchange format, directly usable by other Buf* objects.
 */
struct BufOnsetSlice
{
  halp_meta(name, "BufOnsetSlice")
  halp_meta(c_name, "fluid_bufonsetslice")
  halp_meta(category, "Analysis/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description,
            "Spectral difference onset slicer: segments a sound file and "
            "outputs the slice positions")
  halp_meta(uuid, "D624F4CD-9D0B-4C33-B539-B7324A245519")

  // The non-threaded NRT adaptor is itself a complete client; our worker
  // provides the threading.
  using Client = fluid::client::NRTOnsetSliceClient;
  ...
  struct ins
  {
    ...
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("metric")> metric;
    ...
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("minSliceLength")>
        minSliceLength;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("filterSize")>
        filterSize;
    FluCoMa::fluid_param_for<Client, FluCoMa::pidx<Client>("frameDelta")>
        frameDelta;
  ...
  struct worker_t
  {
    ...
    static std::function<void(BufOnsetSlice&)>
    work(std::shared_ptr<ins> in_ptr, double rate)
    {
      ...
      auto fail = [](std::string msg) {
        return [msg = std::move(msg)](BufOnsetSlice& self) {
      ...
      return [out = std::move(out), f = in.folder.value,
              s = in.source.value](BufOnsetSlice& self) {
