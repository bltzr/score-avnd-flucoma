...
/* Assigns incoming points to the nearest cluster of a trained k-means model,
 * every tick. The model hot-reloads when its file changes on disk — retrain
 * with KMeansFit while playing and predictions follow.
 *
...
struct KMeansPredict
{
  halp_meta(name, "KMeansPredict")
  halp_meta(c_name, "fluid_kmeanspredict")
  ...
  halp_meta(description,
            ...
            "k-means model")
  halp_meta(uuid, "49E1B406-8B08-412C-B493-60C82B52E95E")
  ...
  struct worker_t
  {
    ...
    static std::function<void(KMeansPredict&)>
    work(std::shared_ptr<load_request> rq)
    {
      ...
      if(f.open(QIODevice::ReadOnly))
      {
        try
        {
          ...
          fluid::algorithm::from_json(j, m->kmeans);
      ...
      return [m = std::move(m), err = std::move(err)](KMeansPredict& self) {
  ...
  void operator()(int frames)
  {
    ...
    outputs.cluster.value = (int)model->kmeans.vq(in_buf);
