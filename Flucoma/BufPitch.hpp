...
/* Offline pitch analysis of a sound file read from the working folder.
...
struct BufPitch
{
  halp_meta(name, "BufPitch")
  halp_meta(c_name, "fluid_bufpitch")
  ...
  halp_meta(description,
            ...
            "of a sound file")
  halp_meta(uuid, "AAD01D79-F28C-4A8D-99CD-B16AC12414E2")
  ...
  using Client = fluid::client::NRTPitchClient;

  struct ins
  {
    halp::lineedit<"Folder", ""> folder;
    halp::lineedit<"Source", "source.wav"> source;
    halp::lineedit<"Features file", "pitch.wav"> featuresFile;
  ...
  struct worker_t
  {
    ...
    static std::function<void(BufPitch&)>
    work(std::shared_ptr<ins> in_ptr, double rate)
    {
      ...
      auto fail = [](std::string msg) {
        return [msg = std::move(msg)](BufPitch& self) {
      ...
      return [frames, f = in.folder.value, s = in.source.value](BufPitch& self) {
