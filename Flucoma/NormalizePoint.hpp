...
/* Applies a fitted min/max normalization to incoming points, every tick.
 * The model hot-reloads when its file changes on disk — refit with
 * NormalizeFit while playing and the scaling follows.
 *
 * Feed the input with a feature vector and get the normalized vector out;
 * the Inverse toggle maps normalized values back to the original range.
 */
...
{
  halp_meta(name, "NormalizePoint")
  halp_meta(c_name, "fluid_normalizepoint")
  halp_meta(category, "Analysis/FluCoMa")
  halp_meta(author, "FluCoMa project, ossia team")
  halp_meta(description,
            "Scales incoming points with a fitted min/max normalization")
  halp_meta(uuid, "8F1AE5D8-ED5E-4B94-8FBA-7794F69DCDFE")

  struct ins
  {
    halp::folder_port<"Folder"> folder;
    halp::folder_combobox<"Model file", "Folder", "json", "normalize.json"> model_file;
  ...
  struct worker_t
  {
    ...
    static std::function<void(NormalizePoint&)>
    work(std::shared_ptr<load_request> rq)
    {
      ...
      return [m = std::move(m), err = std::move(err)](NormalizePoint& self) {
